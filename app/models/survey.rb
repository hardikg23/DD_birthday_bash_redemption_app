class Survey < ActiveRecord::Base
  ...
  def self.to_csv
    ...
    CSV.generate do |csv|
      ...
      all.each do |g|
        csv << [g.user_id, g.email, g.city, JSON.parse(g.questions["1"]).join(', '), g.questions["2"], JSON.parse(g.questions["3"]).join(', '), g.questions["4"], JSON.parse(g.questions["5"]).join(', '), g.questions["6"], g.questions["7"], g.questions["8"], g.questions["9"], JSON.parse(g.questions["10"]).join(', '), JSON.parse(g.questions["11"]).join(', '), g.questions["11_other"], JSON.parse(g.questions["11_reason"]).join(', '), g.questions["12"], JSON.parse(g.questions["13"]).join(', '), JSON.parse(g.questions["14"]).join(', '), g.questions["14_wallet"], JSON.parse(g.questions["15"]).join(', '), JSON.parse(g.questions["16"]).join(', '), JSON.parse(g.questions["16_reason"]).join(', '), g.questions["17"], g.questions["18"], g.questions["19"], g.questions["20"], JSON.parse(g.questions["21"]).join(', '), g.questions["22"], g.questions["23"]]
      end
    end
  end
end
